...
bucket_name = "aws-bucketworkshop"
...
def save_file(recipeName, photo):
    photo_extension = photo.filename.split(".")[1]
    photo_name = photo.filename
    photo_path = "/tmp/" + recipeName.replace(" ","") +"."+ photo_extension
    photo.save(photo_path)
    return photo_path

def upload_file_s3(s3_connection, photo_path):
    bucket_name = "aws-bucketworkshop"
    path_s3 = "imagesworkshop/" + photo_path.split("/")[2]
    s3_connection.meta.client.upload_file(photo_path, bucket_name, path_s3)

def get_file_s3(s3_connection, recipename):
    ...
    for object in all_objects:
        ...
        if name_file_s3 == recipename.replace(" ", ""):
            return path_file
    return None